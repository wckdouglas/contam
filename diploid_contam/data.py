from __future__ import annotations

from diploid_contam.models import Interval

EXONS: list[Interval] = [  # hg19
    # GBA
    Interval("1", 155204243, 155204891),
    Interval("1", 155204986, 155205635),
    Interval("1", 155205973, 155206260),
    Interval("1", 155207132, 155207369),
    Interval("1", 155207723, 155208097),
    Interval("1", 155208308, 155208682),
    Interval("1", 155209407, 155209553),
    Interval("1", 155209677, 155209868),
    Interval("1", 155209968, 155210090),
    Interval("1", 155210421, 155210508),
    Interval("1", 155210877, 155211066),
    # CYP21A2
    Interval("6", 32006042, 32007025),
    Interval("6", 32007133, 32007261),
    Interval("6", 32007323, 32007749),
    Interval("6", 32007782, 32007982),
    Interval("6", 32008183, 32008548),
    Interval("6", 32008646, 32009447),
    # PMS2
    Interval("7", 6012870, 6013173),
    Interval("7", 6017219, 6017388),
    Interval("7", 6018227, 6018327),
    Interval("7", 6022455, 6022622),
    Interval("7", 6026390, 6027251),
    Interval("7", 6029431, 6029586),
    Interval("7", 6031604, 6031688),
    Interval("7", 6035165, 6035264),
    Interval("7", 6036957, 6037054),
    Interval("7", 6038739, 6038906),
    Interval("7", 6041597, 6042267),
    Interval("7", 6042941, 6043689),
    Interval("7", 6045523, 6045697),
    Interval("7", 6048628, 6048756),
]
